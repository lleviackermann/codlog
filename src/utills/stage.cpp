...
StagingArea::StagingArea(const std::filesystem::path &index_file_path)
{
    ...
    index_file_read.open(index_file_path, std::ios::in | std::ios::binary);
    ...
    while (std::getline(index_file_read, line))
    {
        ...
        iss >> obj_mode >> obj_hash;
        std::getline(iss >> std::ws, file_path); // Read the rest of the line as file_path
        entries.emplace(file_path, index_entry(obj_mode, obj_hash));
    }

    if (index_file_read.is_open())
        index_file_read.close();
}
