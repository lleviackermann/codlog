...
void create_codlog_directory(const std::string &target_dir)
{
    ...
    const std::filesystem::path main_ref_file_path = codlog_dir / "refs" / "heads" / "main";
    ...
    head_file.close();
    std::ofstream main_ref_file(main_ref_file_path, std::ios::out);
    if (!main_ref_file.is_open())
    {
        throw std::runtime_error("File_path: ./src/commands/init.cpp\nFunction: create_codlog_directory\nError: Unable to create ref/heads/main file");
    }
    std::string initial_commit(40, '0');
    main_ref_file << initial_commit;
    main_ref_file.close();
